...
export const validateWorkspaceName = (workspaceName: string, required: boolean = false): boolean | string => {
  if (!workspaceName || workspaceName.trim() === "") {
    return required ? "Organization name is required" : true;
  }

  if (workspaceName.length > 80) {
    return "Organization name must be 80 characters or less";
  }

  if (hasInjectionRiskChars(workspaceName)) {
    return "Organization name cannot contain special characters like < > ' \" { } [ ] * ^ ! # %";
  }
