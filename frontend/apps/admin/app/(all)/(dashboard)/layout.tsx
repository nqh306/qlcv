...
function AdminLayout(_props: Route.ComponentProps) {
  ...
  // store hooks
  const { isUserLoggedIn, isSuperAdmin, instanceRole } = useUser();

  useEffect(() => {
    if (isUserLoggedIn === false) replace("/");
  }, [replace, isUserLoggedIn]);

  // Workspace Admin route guard: redirect to /users/ if accessing restricted pages
  useEffect(() => {
    if (isUserLoggedIn && instanceRole !== undefined && !isSuperAdmin && pathName && !pathName.includes("/users")) {
      replace("/users/");
    }
  }, [pathName, isSuperAdmin, instanceRole, isUserLoggedIn, replace]);
