...
import { ComingSoonBadge, QLCV_PLANS, PLANS_LIST } from "@/constants/plans";
// local imports
import { PlanFeatureDetail } from "./feature-detail";

type TPlansComparisonBaseProps = {
  planHeaders: React.ReactNode;
...
export const shouldRenderPlanDetail = (planKey: TQlcvPlans) => {
...
export const PlansComparisonBase = observer(function PlansComparisonBase(props: TPlansComparisonBaseProps) {
  const { planHeaders, isSelfManaged, isCompareAllFeaturesSectionOpen, setIsCompareAllFeaturesSectionOpen } = props;
  // plan details
  const { planDetails, planHighlights, planComparison } = QLCV_PLANS;
  const numberOfPlansToRender = Object.keys(planDetails).filter((planKey) =>
    shouldRenderPlanDetail(planKey as TQlcvPlans)
  ).length;

  const getSubscriptionType = (planKey: TQlcvPlans) => planDetails[planKey].id;
