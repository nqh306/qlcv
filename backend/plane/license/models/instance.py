...
class InstanceAdminWorkspace(BaseModel):
    """Maps a workspace-scoped admin (role=20) to the workspace(s) they can manage."""

    instance_admin = models.ForeignKey(
        InstanceAdmin,
    ...
    workspace = models.ForeignKey(
        "db.Workspace",
        on_delete=models.CASCADE,
        related_name="instance_admin_scopes",
    )

    class Meta:
        unique_together = ["instance_admin", "workspace"]
        verbose_name = "Instance Admin Workspace"
        verbose_name_plural = "Instance Admin Workspaces"
        db_table = "instance_admin_workspaces"
        ordering = ("-created_at",)
