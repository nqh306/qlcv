...
from qlcv.db.models import User, Workspace, WorkspaceMember
...
def session_client(api_client, create_user):
    """Return a session authenticated API client for app API testing, which is what qlcv.app uses"""
...
def create_bot_user(db):
    ...
    user = User.objects.create(
        email=f"bot-{unique_id}@evngenco1.vn",
