...
from qlcv.app.permissions import (
...
# Module imports
from qlcv.app.serializers import WorkSpaceSerializer, WorkspaceThemeSerializer
...
from qlcv.app.permissions import ROLE, allow_permission
from qlcv.utils.constants import RESTRICTED_WORKSPACE_SLUGS
from qlcv.license.utils.instance_value import get_configuration_value
from qlcv.license.models import InstanceAdmin
from qlcv.bgtasks.workspace_seed_task import workspace_seed
from qlcv.bgtasks.event_tracking_task import track_event
from qlcv.utils.url import contains_url
from qlcv.utils.analytics_events import WORKSPACE_CREATED, WORKSPACE_DELETED
from qlcv.utils.csv_utils import sanitize_csv_row
...
class WorkSpaceViewSet(BaseViewSet):
    ...
    def create(self, request):
        try:
            # Only Super Admin (InstanceAdmin role>=30) can create workspaces
            if not InstanceAdmin.objects.filter(
                user=request.user, role__gte=30
            ).exists():
                return Response(
            ...
            if serializer.is_valid(raise_exception=True):
                ...
                # Auto-add all Super Admins to this workspace
                try:
                    from qlcv.license.utils.admin_membership import ensure_super_admins_in_workspace
                    workspace = Workspace.objects.get(id=serializer.data["id"])
                    ensure_super_admins_in_workspace(workspace)
                except Exception:
                    pass  # Non-critical: don't block workspace creation
