...
from django.contrib.auth import logout, login, BACKEND_SESSION_KEY, HASH_SESSION_KEY, SESSION_KEY
...
# Module imports
from qlcv.app.serializers import (
    AccountSerializer,
...
from qlcv.app.views.base import BaseAPIView, BaseViewSet
from qlcv.db.models import (
    Account,
    IssueActivity,
    Profile,
    ProjectMember,
...
from qlcv.utils.paginator import BasePaginator
from qlcv.authentication.utils.host import user_ip
from qlcv.bgtasks.user_deactivation_email_task import user_deactivation_email
from qlcv.utils.host import base_host
from qlcv.bgtasks.user_email_update_task import send_email_update_magic_code, send_email_update_confirmation
from qlcv.authentication.rate_limit import EmailVerificationThrottle


logger = logging.getLogger("qlcv")
...
class UserEndpoint(BaseViewSet):
    ...
    def retrieve_instance_admin(self, request):
        from qlcv.license.api.permissions.instance import _is_god_mode_user, _is_super_admin
        # is_instance_admin = can access God Mode (Super Admin OR Workspace Admin via WorkspaceMember role=20)
        is_admin = _is_god_mode_user(request.user)
    ...
    def create_admin_session(self, request):
        """Bridge web session to admin session for SSO into god-mode.
        This endpoint is NOT under /instances/ path, so the middleware
        authenticates via the regular session-id cookie. We then manually
        create an admin session and set the admin-session-id cookie."""
        ...
        if not instance:
            return Response({"error": "Instance not configured"}, status=status.HTTP_400_BAD_REQUEST)

        from qlcv.license.api.permissions.instance import _is_god_mode_user
        if not _is_god_mode_user(request.user):
            return Response({"error": "Not an instance admin"}, status=status.HTTP_403_FORBIDDEN)

        # Create a new admin session manually
        engine = import_module(settings.SESSION_ENGINE)
        admin_session = engine.SessionStore()
        admin_session.create()

        # Populate session with auth data (same as django.contrib.auth.login)
        admin_session[SESSION_KEY] = str(request.user.pk)
        backend = request.session.get(BACKEND_SESSION_KEY, "django.contrib.auth.backends.ModelBackend")
        admin_session[BACKEND_SESSION_KEY] = backend
        admin_session[HASH_SESSION_KEY] = request.user.get_session_auth_hash()
        admin_session.set_expiry(settings.ADMIN_SESSION_COOKIE_AGE)
        admin_session["device_info"] = {
        ...
        # Build response and set the admin-session-id cookie
