...
# Module imports
from qlcv.db.models import User, Workspace, WorkspaceMemberInvite
from qlcv.license.utils.instance_value import get_email_configuration
from qlcv.utils.email import generate_plain_text_from_html
from qlcv.utils.exception_logger import log_exception
...
def workspace_invitation(email, workspace_id, token, current_site, inviter):
    try:
        ...
        subject = f"{user.first_name or user.display_name or user.email} has invited you to join them in {workspace.name} trên QLCV"  # noqa: E501
        ...
        msg.send()
        logging.getLogger("qlcv.worker").info("Email sent successfully")
        return
