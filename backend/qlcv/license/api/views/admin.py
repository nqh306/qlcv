...
from qlcv.license.api.permissions import InstanceAdminPermission
from qlcv.license.api.permissions.instance import _is_god_mode_user, _is_super_admin
from qlcv.license.api.serializers import (
    InstanceAdminMeSerializer,
    InstanceAdminSerializer,
)
from qlcv.license.models import Instance, InstanceAdmin
from qlcv.license.utils.admin_membership import ensure_admin_in_all_workspaces
from qlcv.db.models import User, Profile, WorkspaceMember
from qlcv.utils.cache import cache_response, invalidate_cache
from qlcv.authentication.utils.login import user_login
from qlcv.authentication.utils.host import base_host, user_ip
from qlcv.authentication.adapter.error import (
    AUTHENTICATION_ERROR_CODES,
    AuthenticationException,
)
from qlcv.utils.ip_address import get_client_ip
...
class InstanceAdminEndpoint(BaseAPIView):
    ...
    def post(self, request):
        ...
        # Only Super Admin (role=30) can be created via this endpoint
        # Workspace Admin is determined by WorkspaceMember role=20
        instance_admin = InstanceAdmin.objects.create(instance=instance, user=user, role=30)
...
class InstanceAdminSignInEndpoint(View):
    ...
    def post(self, request):
        ...
        # Check if user can access God Mode: Super Admin or Workspace Admin (WorkspaceMember role=20)
        if not _is_god_mode_user(user):
            exc = AuthenticationException(
                error_code=AUTHENTICATION_ERROR_CODES["ADMIN_AUTHENTICATION_FAILED"],
                error_message="ADMIN_AUTHENTICATION_FAILED",
                payload={"email": email},
...
class InstanceAdminUserMeEndpoint(BaseAPIView):
    ...
    def get(self, request):
        ...
        # Determine role: Super Admin (InstanceAdmin role>=30) or Workspace Admin (WorkspaceMember role=20)
        if _is_super_admin(request.user):
            data["instance_role"] = 30
            data["scoped_workspace_ids"] = []
        else:
            data["instance_role"] = 20
...
class InstanceAdminUserSessionEndpoint(BaseAPIView):
    ...
    def get(self, request):
        if request.user.is_authenticated and _is_god_mode_user(request.user):
            serializer = InstanceAdminMeSerializer(request.user)
