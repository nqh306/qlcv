...
from qlcv.license.api.permissions import InstanceAdminOrScopedPermission, InstanceAdminPermission
from qlcv.license.api.serializers import InstanceUserListSerializer, InstanceUserDetailSerializer
...
from qlcv.db.models import User, Profile, Workspace, WorkspaceMember, Project, ProjectMember
...
class InstanceUserManagementEndpoint(BaseAPIView):
    ...
    def get(self, request, pk=None):
        scoped_ws_ids = request.scoped_workspace_ids
    ...
    def _set_admin_role(self, request, pk):
        ...
        new_role = request.data.get("role")  # 30 (Super Admin) or null (remove Super Admin)
        ...
        if new_role != 30:
            return Response(
                {"error": "Invalid role. Use 30 (Super Admin) or null (remove). Workspace Admin is set via WorkspaceMember role."},
                status=status.HTTP_400_BAD_REQUEST,
            )

        if existing_admin:
            existing_admin.role = 30
            existing_admin.save(update_fields=["role"])
        else:
            InstanceAdmin.objects.create(instance=instance, user=user, role=30)
